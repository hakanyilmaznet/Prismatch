...
function google_badge_html(): string {
  return '<span class="badge text-bg-light border text-dark d-inline-flex align-items-center gap-1">'
    . '<img src="google.svg" alt="Google" width="14" height="14" />'
    . '<span>Google</span>'
    . '</span>';
}
...
<main class="container py-4 d-grid gap-3 app" aria-label="<?= htmlspecialchars(tt('app_name', 'Prismatch')) ?>">

  <section class="d-flex flex-wrap gap-2" aria-label="<?= htmlspecialchars(tt('app_name', 'Prismatch')) ?>">
    <div class="card flex-fill p-2 text-center" role="status" aria-live="polite">
  ...
  <section class="card shadow-sm p-3 text-center stage" id="stage" aria-label="<?= htmlspecialchars(tt('app_name', 'Prismatch')) ?>">
    <div class="alert alert-info py-2 mb-3 d-none" id="toast" aria-live="polite"></div>

    <div class="d-grid gap-2 text-center" id="center">
      <img src="logo.svg" alt="<?= htmlspecialchars(tt('app_name', 'Prismatch')) ?> logo" width="84" height="84" class="rounded shadow-sm" />
      <div class="h4 mb-1"><?= htmlspecialchars(tt('app_name', 'Prismatch')) ?></div>
      ...
      <?php if ($flash): ?>
        <div class="text-body-secondary"><strong><?= htmlspecialchars($flash) ?></strong></div>
      <?php endif; ?>
      <?php if ($isDailyMode): ?>
        <div class="text-body-secondary"><strong><?= htmlspecialchars(tt('daily_once', 'Daily challenge: one attempt per day.')) ?></strong></div>
      <?php endif; ?>

      <?php if ($userEmail && $stats): ?>
        <div class="row g-2" aria-label="<?= htmlspecialchars(tt('stats_title', 'Stats')) ?>">
          <div class="col-6"><div class="text-body-secondary small"><?= htmlspecialchars(tt('stats_total_games', 'Total Games')) ?></div><div class="fw-semibold"><?= (int)$stats['total_plays'] ?></div></div>
          <div class="col-6"><div class="text-body-secondary small"><?= htmlspecialchars(tt('stats_total_wins', 'Total Wins')) ?></div><div class="fw-semibold"><?= (int)$stats['total_wins'] ?></div></div>
          <div class="col-6"><div class="text-body-secondary small"><?= htmlspecialchars(tt('stats_best_level', 'Best Stage')) ?></div><div class="fw-semibold"><?= (int)$stats['best_level'] ?></div></div>
          <div class="col-6"><div class="text-body-secondary small"><?= htmlspecialchars(tt('stats_total_correct', 'Total Correct')) ?></div><div class="fw-semibold"><?= (int)$stats['total_correct'] ?></div></div>
        </div>

        <div class="d-flex flex-wrap gap-2 justify-content-center">
          <a class="btn btn-outline-secondary" href="games.php">
            <img width="16" height="16" class="me-2" src="bootstrap-icons/clock-history.svg" alt="" aria-hidden="true" />
            <?= htmlspecialchars(tt('btn_view_history', 'My Sessions')) ?>
          </a>
          <?php if ($isDailyMode): ?>
          <a class="btn btn-outline-secondary" href="daily_leaderboard.php">
            <img width="16" height="16" class="me-2" src="bootstrap-icons/trophy-fill.svg" alt="" aria-hidden="true" />
            <?= htmlspecialchars(tt('daily_leaderboard_title', 'Leaderboard')) ?>
          </a>
          <?php endif; ?>
          <a class="btn btn-outline-secondary" href="logout.php">
            <img width="16" height="16" class="me-2" src="bootstrap-icons/box-arrow-right.svg" alt="" aria-hidden="true" />
      ...
      <button class="btn btn-primary" id="btnStart" type="button">
        <img width="16" height="16" class="me-2" src="bootstrap-icons/play-fill.svg" alt="" aria-hidden="true" />
        <?= htmlspecialchars(tt('btn_start', 'Hemen Oyna')) ?>
      </button>
    </div>
  </section>

  <section class="d-flex justify-content-between align-items-center gap-2" aria-label="<?= htmlspecialchars(tt('controls', 'Controls')) ?>">
...
<div class="position-fixed top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center bg-dark bg-opacity-50" id="answerOverlay" hidden role="dialog" aria-modal="true" aria-live="polite">
  <div class="card shadow-sm text-center p-3" id="answerCard">
    ...
    <p class="text-body-secondary mb-0"><?= htmlspecialchars(tt('badge_answer', 'Answer now!')) ?></p>
  </div>
</div>

<div class="position-fixed top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center bg-dark bg-opacity-50" id="dailyCompletedModal" hidden role="dialog" aria-modal="true" aria-labelledby="dailyCompletedTitle">
  <div class="card shadow-sm text-center p-3">
    <h2 class="h5 mb-2" id="dailyCompletedTitle"><?= htmlspecialchars(tt('daily_title', 'Daily Challenge')) ?></h2>
    <p class="text-body-secondary" id="dailyCompletedText"><?= htmlspecialchars(tt('daily_completed', 'You already played today. Come back tomorrow!')) ?></p>
    <div class="d-flex justify-content-center gap-2">
      <button class="btn btn-primary d-inline-flex align-items-center gap-2" id="dailyCompletedOk" type="button">
        <img width="16" height="16" src="bootstrap-icons/check-circle.svg" alt="" aria-hidden="true" />
...
<div class="position-fixed top-0 start-0 w-100 h-100 d-flex align-items-center justify-content-center bg-dark bg-opacity-50" id="dailyLoginModal" hidden role="dialog" aria-modal="true" aria-labelledby="dailyLoginTitle">
  <div class="card shadow-sm text-center p-3">
    <h2 class="h5 mb-2" id="dailyLoginTitle"><?= htmlspecialchars(tt('daily_title', 'Daily Challenge')) ?></h2>
    <p class="text-body-secondary" id="dailyLoginText"><?= htmlspecialchars(tt('daily_login_required', 'Log in to play the daily challenge.')) ?></p>
    <div class="d-flex justify-content-center gap-2">
      <a class="btn btn-primary d-inline-flex align-items-center gap-2" id="dailyLoginGo" href="login.php">
        <img width="16" height="16" src="bootstrap-icons/box-arrow-in-right.svg" alt="" aria-hidden="true" />
        <?= htmlspecialchars(tt('home_cta_login', 'Sign in')) ?>
      </a>
      <button class="btn btn-outline-secondary d-inline-flex align-items-center gap-2" id="dailyLoginBack" type="button">
        <img width="16" height="16" src="bootstrap-icons/arrow-left.svg" alt="" aria-hidden="true" />
...
function showSavePromptModal(payload){
  ...
  modal.className = "card shadow-sm text-center p-3";
  ...
  actions.className = "d-flex justify-content-center gap-2 flex-wrap";

  const btnSave = document.createElement("button");
  btnSave.className = "btn btn-primary d-inline-flex align-items-center gap-2";
  btnSave.type = "button";
    btnSave.innerHTML = '<img width="16" height="16" src="bootstrap-icons/cloud-arrow-up.svg" alt="" aria-hidden="true" /> ' +
  ...
  btnSkip.className = "btn btn-outline-secondary d-inline-flex align-items-center gap-2";
  btnSkip.type = "button";
    btnSkip.innerHTML = '<img width="16" height="16" src="bootstrap-icons/arrow-right.svg" alt="" aria-hidden="true" /> ' + tjs('continue_without_saving', 'Continue without saving');
...
async function win(){
  ...
  if (!IS_DAILY_MODE){
    ...
    btn.type = "button";
      btn.innerHTML = '<img width="16" height="16" class="me-2" src="bootstrap-icons/arrow-repeat.svg" alt="" aria-hidden="true" /> ' + tjs('btn_play_again', 'Play again');
  ...
  if (IS_LOGGED_IN){
    ...
    a.className = "btn";
    a.href = "games.php";
    a.innerHTML = '<img width="16" height="16" class="me-2" src="bootstrap-icons/clock-history.svg" alt="" aria-hidden="true" /> ' + tjs('btn_view_history', 'My Sessions');
    row.appendChild(a);
  }
...
async function gameOver(reason){
  ...
  if (!IS_DAILY_MODE){
    ...
    btn.className = "btn primary";
    btn.type = "button";
      btn.innerHTML = '<img width="16" height="16" class="me-2" src="bootstrap-icons/arrow-clockwise.svg" alt="" aria-hidden="true" /> ' + tjs('btn_restart', 'Restart');
    btn.addEventListener("click", startGame);
    row.appendChild(btn);
  }
